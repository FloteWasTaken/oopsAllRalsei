...
extern MY_HIGHP_OR_MEDIUMP vec2 lover;
...
vec4 dissolve_mask(vec4 tex, vec2 texture_coords, vec2 uv)
{
    ...
    bool visible = res > adjusted_dissolve;
    vec3 final_rgb = shadow ? vec3(0.0) : (visible ? tex.rgb : vec3(0.0));
    float final_alpha = visible ? (shadow ? tex.a * 0.3 : tex.a) : 0.0;
...
float fade_factor = clampf2(heart_list); // 0 to 1
    ...
    vec3 heart_rgb = RGB(hsl).rgb;
    tex.rgb = mix(tex.rgb, heart_rgb, clampf2(heart_list));

    pixel = vec4(pixel.rgb * 0.0 + tex.rgb * tex.a, tex.a);



    float res = (.5 + .5* cos( (lover.x) * 2.612 + ( field + -.5 ) *3.14));
    vec4 textp = RGB(hsl);
    tex.rgb = textp.rgb;
